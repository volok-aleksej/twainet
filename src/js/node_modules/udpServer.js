...
exports.udpServer.prototype.Start = function(pbBuilder) {
	...
	var sendInitTunnel = function(address) {
		var initTunnel = self.pbBuilder.build("client_server.InitTunnel"),
			tunnelConnectAddress = self.pbBuilder.build("client_server.TunnelConnectAddress"),
			tunnelType = self.pbBuilder.build("client_server.TunnelType");

		var myMessage = new initTunnel(self.extSessionId, self.ownSessionId, tunnelType.TUNNEL_EXTERNAL, new tunnelConnectAddress(address.address, address.port));
		self.module.notifyMessage(0, myMessage);
	}

	var sendTryConnectTo = function(ownSessionId, extSessionId, address) {
		var tryConnectTo = self.pbBuilder.build("client_server.TryConnectTo"),
			tunnelConnectAddress = self.pbBuilder.build("client_server.TunnelConnectAddress"),
			tunnelType = self.pbBuilder.build("client_server.TunnelType");
	...
	server.on("listening", function () {
		...
		sendInitTunnel({address: addr, port: address.port});
	});
	...
	server.on('message', function (data, rinfo) {
		if(state == stateEnum.LENGTH_DATA) {
			...
			if(moduleName == self.ownSessionId) {
				sendTryConnectTo(self.extSessionId, moduleName, rinfo);
				server.close();
			}
		}
	});

...
exports.Module.prototype.addConnector = function(connector, c, id, moduleName) {
	var conn = new connector(c, id, moduleName);
	conn.module = this;
...
exports.Module.prototype.startConnector = function(conn) {
	conn.start();
}
...
exports.Module.prototype.changeConnectorId = function(oldId, newId) {
	if(this.connMap[oldId] !== undefined) {
		...
		if(this.connMap[newId] == undefined) {
			this.connMap[newId] = conn;
		} else {
			this.connMap[newId] = this.connMap[newId].concat(conn);
		}
	}
}

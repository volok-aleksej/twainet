...
var moduleNameToId = function(ipcName) {
	if(ipcName.get("host_name") != "" && ipcName.get("suffix") != "")
		return ipcName.get("module_name") + "->" + ipcName.get("host_name") + "->" + ipcName.get("suffix");
	else if(ipcName.get("host_name") != "")
		return ipcName.get("module_name") + "->" + ipcName.get("host_name");
	else
		return ipcName.get("module_name");
}

var moduleNameFromId = function(id, builder) {
	var ipcNamePrototype = builder.build("ipc.IPCName"),
...
exports.IPCConnector = function(c, id, moduleName) {
	var protoFilePath = "../messages/ipc.proto",
		self = this,
		protoMap = {},
		msgMap = {};

	conn.Connector.call(this, c, id, moduleName);

	this.builder = protobuf.loadProtoFile(protoFilePath);
	this.isExist = false;
	this.rndval = utils.rand(0, 100).toString();
	this.isCoordinator = (moduleName == module.ipcCoordinatorName);
	...
	handlers.prototype.ModuleName = function(object) {
		...
		console.info("ModuleName: ", moduleNameToId(ipcName));
		...
		for(moduleName in moduleList) if(moduleList.hasOwnProperty(moduleName)) {
			...
			var addIPCObject = self.builder.build("ipc.AddIPCObject"),
				ipcModuleName = moduleNameFromId(moduleName, self.builder),
	...
	handlers.prototype.ModuleState = function(object) {
		...
		if(self.isExist && exist) {
			...
			var moduleStatePrototype = self.builder.build("ipc.ModuleState"),
				moduleState = new moduleStatePrototype(false, randomValue, self.id);

			self.module.notifyMessage(self, moduleState);
			exist = moduleState.get("exist");
			randomValue = moduleState.get("rndVal");
	...
	handlers.prototype.IPCObjectList = function(object) {
		...
		for(var i = 0; i < ipcObjects.length; i++) {
			handlers.prototype.AddIPCObject(ipcObjects[i]);
		}
	}
	...
	handlers.prototype.ChangeIPCName = function(object) {
		...
		console.info("change connector id", moduleNameToId(ipcName));
	...
	notifyHandlers.prototype.ChangeIPCName = function(object) {
		var newIPCName = object.get("ipc_name"),
			updateIPCObjectPrototype = self.builder.build("ipc.UpdateIPCObject"),
			updateIPCObject = new updateIPCObjectPrototype(moduleNameFromId(self.id), newIPCName);

			self.sendMessage(updateIPCObject);
	}
...
exports.IPCConnector.prototype.changeModuleName = function(moduleName) {
	console.info("Change module name", moduleName);
	this.moduleName = moduleName;
	var changeIPCNamePrototype = this.builder.build("ipc.ChangeIPCName"),
		changeIPCName = new changeIPCNamePrototype(moduleNameFromId(this.moduleName, this.builder));

	this.sendMessage(changeIPCName);
}

exports.IPCConnector.prototype.start = function() {
	var moduleNamePrototype = this.builder.build("ipc.ModuleName");
		moduleName = new moduleNamePrototype(moduleNameFromId(this.moduleName, this.builder), this.module.ip, this.module.port);
...
exports.IPCConnector.prototype.destroy = function() {
	var removeIPCObjectPrototype = this.builder.build("ipc.RemoveIPCObject");
		removeIPCObject = new removeIPCObjectPrototype(this.moduleName);

	conn.Connector.prototype.destroy.call(this);

	this.module.notifyMessage(this, removeIPCObject);
}